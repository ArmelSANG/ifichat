...
var CFG={},CONV=null,VID=null,MSGS=[],OPEN=false,POLL=null,ERR_COUNT=0;
...
function buildUI(){
  ...
  win.innerHTML='<div id="ific-hdr"><div><div id="ific-hdr-name">'+(CFG.business_name||"Chat")+'</div><div id="ific-hdr-status">En ligne</div></div><button id="ific-close">&#10005;</button></div><div id="ific-msgs"><div id="ific-welc">'+(CFG.welcome_message||"Bonjour ! Comment pouvons-nous vous aider ?")+'</div></div><div id="ific-form-wrap"><form id="ific-form"><label>Nom</label><input id="ific-fn" placeholder="Votre nom" required><label>WhatsApp / T\u00e9l\u00e9phone</label><input id="ific-wa" placeholder="+229..." required><button type="submit">D\u00e9marrer le chat</button></form></div><div id="ific-inp"><form id="ific-sf"><input id="ific-si" placeholder="'+(CFG.placeholder_text||"\u00c9crivez votre message...")+'" autocomplete="off"><button type="submit"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></button></form></div>';
  document.body.appendChild(win);

  document.getElementById("ific-close").onclick=function(){OPEN=false;win.style.display="none";stopPolling()};
...
function toggleChat(){
  ...
  if(OPEN){
    if(!CONV){document.getElementById("ific-form-wrap").style.display="block"}
    else{startPolling()}
    var b=document.getElementById("ific-badge");b.style.display="none";b.textContent="0";
  }else{stopPolling()}
}
...
function startConv(e){
  ...
  fetch(API+"/start-conversation",{method:"POST",headers:{"Content-Type":"application/json","x-client-id":CID},
    ...
    if(d.conversationId){
      ...
      try{localStorage.setItem("ific_"+CID,JSON.stringify({conv:CONV,vid:VID}))}catch(x){}
      startPolling();
...
function startPolling(){
  ...
  POLL=setInterval(function(){
    if(!OPEN||ERR_COUNT>5){stopPolling();return}
    pollNow();
  },4000);
}
...
function pollNow(){
  fetch(API+"/messages/"+CONV).then(function(r){
    if(!r.ok){ERR_COUNT++;return null}
    ERR_COUNT=0;return r.json();
  }).then(function(d){
    ...
    d.messages.forEach(function(m){
      if(!MSGS.some(function(e){return e.id===m.id})){
        MSGS.push(m);
        if(m.sender==="client"){addBubble(m.content,"mv");newCount++}
      }
    });
    if(!OPEN&&newCount>0){
      var b=document.getElementById("ific-badge");
      b.textContent=String(parseInt(b.textContent||"0")+newCount);
      b.style.display="flex";
    }
  }).catch(function(){ERR_COUNT++});
}
...
function restoreSession(){
  try{
    ...
    if(saved&&saved.conv){
      ...
      // Don't start polling until chat is opened
    }
  }catch(x){}
}
